...
public class SicHTMLToMap {


    private final SicHTML converter;
    private Map returnFinal;

    public SicHTMLToMap(SicHTML converter) {
    ...
    private Map returnMap() {
        ...
        returnMap.setBOX_COLOR((returnMap.getBOX_COLOR().equals(this.converter.getBOX_COLOR())
                || this.converter.getBOX_COLOR().isEmpty()) ? returnMap.getBOX_COLOR() : this.converter.getBOX_COLOR());
        ...
        for (Square item : this.converter.getListSquare()) {
            boolean primera =item.getPossY() >= 0;
            boolean segundo = item.getPossY() < returnMap.getRows();
            boolean tercero = item.getPossX() >= 0;
            boolean cuarto = item.getPossX() < returnMap.getCols();

            if (
                item.getPossY() >= 0 && item.getPossY() < returnMap.getRows() && item.getPossX() >= 0
                    && item.getPossX() < returnMap.getCols()
                    ) {
                if ((
                    returnMap.getMatrix().get(item.getPossY()).get(item.getPossX()).getType() == ListSquare.HALL ||
                    returnMap.getMatrix().get(item.getPossY()).get(item.getPossX()).getType() == ListSquare.UNDEFINED)
                    ) {
                    switch (item.getType()) {
                        case BOX:
                            ...
                            break;
                        case BRICK:
                            returnMap.getMatrix().get(item.getPossY()).get(item.getPossX()).setType(ListSquare.BRICK);
                            break;
                        case TARGET:
                            returnMap.getMatrix().get(item.getPossY()).get(item.getPossX()).setType(ListSquare.TARGET);
                            break;
                        case HALL:
                            returnMap.getMatrix().get(item.getPossY()).get(item.getPossX()).setType(ListSquare.HALL);
                            break;
                        case PLAY:
                            returnMap.getMatrix().get(item.getPossY()).get(item.getPossX()).setType(ListSquare.PLAY);
                            break;
                        default:
                            break;
                    }
